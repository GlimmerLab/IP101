...
void test_video_inpaint(const string& video_path, const Mat& mask) {
    ...
    for(int i = 0; i < 5; i++) {  // Read 5 frames
        Mat frame;
        if(cap.read(frame)) {
    ...
    if(frames.empty()) {
        cout << "Cannot read video frames" << endl;
        return;
    }
    ...
    auto end = chrono::high_resolution_clock::now();
    auto duration = chrono::duration_cast<chrono::milliseconds>(end - start);
    cout << "Video inpainting processing time: " << duration.count() << "ms" << endl;

    // Display results
    for(size_t i = 0; i < results.size(); i++) {
        imshow("Original frame " + to_string(i), frames[i]);
        imshow("Inpainted result " + to_string(i), results[i]);
    }
    waitKey(0);
...
int main() {
    ...
    // Create different types of test masks
